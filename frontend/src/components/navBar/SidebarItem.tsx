...
export default function SidebarItem({ icon, active = false, text, expanded = false, subMenu = null }: SidebarItemProps) {
    ...
    return (
        <>
            <li>
                <button
                    className={`
                        ...
                        ${active && !subMenu ? 'text-primary-500 bg-gradient-to-tr from-indigo-200 to-indigo-100' : 'text-gray-600 hover:bg-indigo-50'}
            ...
            <ul className={`pl-6 ${expandSubMenu ? 'block' : 'hidden'}`}>
                {expanded &&
                    subMenu?.map((item, index) => (
